...
function w3vx_vote_buttons_wrapper($id, $type = "post", $icon = "chevron", $orientation = null){
	...
	//if user submitted post or comment, disable vote functionality
	if( ($type == "post" && get_post($id) !== null && intval(get_post($id)->post_author) == $userID) || ($type == "comment" && get_comment($id, OBJECT)->user_id == $userID) ) {
	   $disable = " disable";
	...
	if($type == "post" && $orientation == null){
		$orientation = "vertical";
	} elseif($type == "comment" && $orientation == null){
		$orientation = "horizontal";
	}
		
	if($orientation == "vertical"){
		$html = 
			'<div class="btn-group vote-buttons-wrapper '.$type.'-vote '.$orientation.' '.$icon.'" role="group" aria-label="Vote Buttons" id="voteButton-'.$id.'" data-id="'.$id.'" data-type="'.$type.'">
				<div class="vote-button vote-up '.$disable.' '.$active1.'" data-vote="up" data-pid="'.$id.'"  data-type="'.$type.'"></div>
				<div class="vote-count" id="'.$type.'VoteCount-'.$id.'"></div>			
				<div class="vote-button vote-down '.$disable.' '.$active0.'" data-vote="down" data-pid="'.$id.'"  data-type="'.$type.'"></div>
			</div>				
			';
	} elseif($orientation == "horizontal"){
		//IMPORTANT: for horizontal vote wrappers, there needs to be a non-breaking space (&nbsp;) for in the buttons or else the buttons won't align with the vote count. 
		$html = 
			'<div class="vote-buttons-wrapper '.$type.'-vote '.$orientation.' '.$icon.'" aria-label="Vote Buttons" id="voteButton-'.$id.'" data-id="'.$id.'" data-type="'.$type.'">
				<span class="vote-count"  id="'.$type.'VoteCount-'.$id.'"></span>			
...
function w3vx_get_vote_data(){
	...
	foreach($ids as $id){
		...
		if( ($type == "post" && intval(get_post($id)->post_author) == $userID) || ($type == "comment" && get_comment($id, OBJECT)->user_id == $userID) ) {
			$data[$id]["active"] = "disable";	
		} else {
			$vote = w3vx_get_user_vote($id, $userID, $type);
	
			$data[$id]["active"] = isset($vote) && !empty($vote) ? $vote : null;
		}
	}	
...
add_action( 'wp_ajax_nopriv_w3vx_get_vote_data', 'w3vx_get_vote_data' );
add_action( 'wp_ajax_w3vx_get_vote_data', 'w3vx_get_vote_data' );
